...
/**
 * 项目：   5anet(BBS)
 * 文件：   class/bbs/ListUser.class.php
 *
 * 列出论坛用户
 *
 * PHP Version 5
 ...
 * @version:   $Id: ListUser.class.php,v 1.1.1.1 2006-08-28 13:09:20 ghw Exp $
...
if ( file_exists(LANG_PATH.SYSTEM_LANG.'/ListUser.lang.php') ) {
   include_once LANG_PATH.SYSTEM_LANG.'/ListUser.lang.php';
}



class ListUser extends BaseAction {
   ...
   public function run() {
      ...
      //求用户的情况
      $sql = 'select a.id, a.user_name, a.register_date, b.group_name from base_user_info a join '.
      ...
      while ( $rows = $res->FetchRow() ) {
         ...
         $temp_sth = $this->db->Prepare($temp_sql);
         $temp_res = $this->db->SelectLimit($temp_sth, 1, 0, array($rows['id']));
         $temp_rows = $temp_res->FetchRow();
