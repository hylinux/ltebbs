...
/**
 * 项目：   5anet(BBS)
 * 文件：   class/message/InBox.class.php
 *
 * 显示用户的短消息收件箱
 *
 * PHP Version 5
 ...
 * @version:   $Id: InBox.class.php,v 1.1.1.1 2006-08-28 13:09:20 ghw Exp $
...
if ( file_exists(LANG_PATH.SYSTEM_LANG.'/InBox.lang.php') ) {
   include_once LANG_PATH.SYSTEM_LANG.'/InBox.lang.php';
}


class InBox extends BaseAction {
   /**
    * 数据库的连接
    */
   public $db;
   ...
   public function run() {
      ...
      $sql = 'select a.id, a.send_user_id, b.user_name, a.title, a.receive_time, '.
         ' a.is_read from message_inbox '.
         ' as a  left join base_user_info as b on a.send_user_id = b.id '.
      ...
      while ( $rows = $res->FetchRow() ) {
         ...
         $temp_array[] = array (
            ...
            'send_user_id' => $rows['send_user_id'],
            'send_user_name' => $send_user_name,
            'title' => $rows['title'],
            'short_title' => utf8_substr($rows['title'], 0, 18),
            'send_date' => $rows['receive_time']
         );
      }
