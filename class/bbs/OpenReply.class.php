...
/**
 * 项目：   5anet(BBS)
 * 文件：   class/bbs/OpenReply.class.php
 *
 * 解锁一个用户回复
 * 在操作之前之前要判断用户的身份。
 * 登录不需判断。
 ...
 * @version:   $Id: OpenReply.class.php,v 1.1.1.1 2006-08-28 13:09:20 ghw Exp $
...
if ( file_exists(LANG_PATH.SYSTEM_LANG.'/OpenReply.lang.php') ) {
   include_once LANG_PATH.SYSTEM_LANG.'/OpenReply.lang.php';
}

class OpenReply extends BaseAction {
   ...
   public function run() {
      ...
      if ( !$id ) {
         $this->AlertAndBack(OR_ID_IS_EMPTY);
         return;
      }

      //验证主题是否存在
      if ( !TopicUtil::replyIsExists($this->db, $id) ) {
         $this->AlertAndBack(OR_ID_IS_NOT_EXISTS);
         return;
      }
      ...
      if ( $user_group != 1 && $user_group != 2 && $user_group != 3 ) {
         //用户没有权限解锁回复
         $this->AlertAndBack(OR_USER_HAVE_NO_PRIVILEGES);
         return;
      }
      ...
      $sql = 'update bbs_reply set reply_status=0 where id=?';
      $sth = $this->db->Prepare($sql);
