...
/**
 * 项目：   5anet(BBS)
 * 文件：   class/bbs/UnSetTop.class.php
 *
 * 取消一个主题的置顶设置
 * 在设置主题之前要判断用户的身份。
 * 登录不需判断。
 ...
 * @version:   $Id: UnSetTop.class.php,v 1.1.1.1 2006-08-28 13:09:20 ghw Exp $
...
if ( file_exists(LANG_PATH.SYSTEM_LANG.'/UnSetTop.lang.php') ) {
   include_once LANG_PATH.SYSTEM_LANG.'/UnSetTop.lang.php';
}

class UnSetTop extends BaseAction {
   ...
   public function run() {
      ...
      if ( !$id ) {
         $this->AlertAndBack(UT_ID_IS_EMPTY);
         return;
      }

      //验证主题是否存在
      if ( !TopicUtil::isExists($this->db, $id) ) {
         $this->AlertAndBack(UT_ID_IS_NOT_EXISTS);
         return;
      }
      ...
      if ( $user_group != 1 && $user_group != 2 && $user_group != 3 ) {
         //用户就没有权限设置主题
         $this->AlertAndBack(UT_USER_HAVE_NO_PRIVILEGES);
         return;
      }
      ...
      $sql = 'update bbs_subject set is_top=0 where id=?';
      $sth = $this->db->Prepare($sql);
      $this->db->Execute($sth, array($id));

      //成功后，则转向
      $this->TipsAndForward(
          ST_OPERA_SUCCESS_LABEL,
