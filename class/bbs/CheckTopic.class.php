...
/**
 * 项目：   5anet(BBS)
 * 文件：   class/bbs/CheckTopic.class.php
 *
 * 给一个主题加上认证
 * 在加认证主题之前要判断用户的身份。
 * 登录不需判断。
 ...
 * @version:   $Id: CheckTopic.class.php,v 1.1.1.1 2006-08-28 13:09:20 ghw Exp $
...
if ( file_exists(LANG_PATH.SYSTEM_LANG.'/CheckTopic.lang.php') ) {
   include_once LANG_PATH.SYSTEM_LANG.'/CheckTopic.lang.php';
}

class CheckTopic extends BaseAction {
   ...
   public function run() {
      ...
      //用户是超级版主
      //用户是系统管理员
      $sql = 'update bbs_subject set subject_status=1 where id=?';
      $sth = $this->db->prepare($sql);
      ...
      $this->forward('index.php?module=bbs&action=viewtopic&id='.$id);
      return;
