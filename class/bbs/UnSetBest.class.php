...
/**
 * 项目：   5anet(BBS)
 * 文件：   class/bbs/UnSetBest.class.php
 *
 * 取消一个精华主题的设置
 * 在设置主题之前要判断用户的身份。
 * 登录不需判断。
 ...
 * @version:   $Id: UnSetBest.class.php,v 1.1.1.1 2006-08-28 13:09:20 ghw Exp $
...
if ( file_exists(LANG_PATH.SYSTEM_LANG.'/UnSetBest.lang.php') ) {
   include_once LANG_PATH.SYSTEM_LANG.'/UnSetBest.lang.php';
}

class UnSetBest extends BaseAction {
   ...
   public function run() {
      ...
      if ( !$id ) {
         $this->AlertAndBack(UB_ID_IS_EMPTY);
         return;
      }

      //验证主题是否存在
      if ( !TopicUtil::isExists($this->db, $id) ) {
         $this->AlertAndBack(UB_ID_IS_NOT_EXISTS);
         return;
      }
      ...
      if ( $user_group != 1 && $user_group != 2 && $user_group != 3 ) {
         //用户就没有权限设置主题
         $this->AlertAndBack(UB_USER_HAVE_NO_PRIVILEGES);
         return;
      }
      ...
      $sql = 'update bbs_subject set is_best=0 where id=?';
      $sth = $this->db->Prepare($sql);
