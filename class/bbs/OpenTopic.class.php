...
/**
 * 项目：   5anet(BBS)
 * 文件：   class/bbs/OpenTopic.class.php
 *
 * 打开一个主题
 * 在打开主题之前要判断用户的身份。
 * 登录不需判断。
 ...
 * @version:   $Id: OpenTopic.class.php,v 1.1.1.1 2006-08-28 13:09:20 ghw Exp $
...
if ( file_exists(LANG_PATH.SYSTEM_LANG.'/OpenTopic.lang.php') ) {
   include_once LANG_PATH.SYSTEM_LANG.'/OpenTopic.lang.php';
}

class OpenTopic extends BaseAction {
   ...
   public function run() {
      ...
      if ( !$id ) {
         $this->AlertAndBack(OT_ID_IS_EMPTY);
         return;
      }

      //验证主题是否存在
      if ( !TopicUtil::isExists($this->db, $id) ) {
         $this->AlertAndBack(OT_ID_IS_NOT_EXISTS);
         return;
      }
      ...
      if ( $user_group != 1 && $user_group != 2 && $user_group != 3 ) {
         //用户就没有权限打开主题
         $this->AlertAndBack(OT_USER_HAVE_NO_PRIVILEGES);
         return;
      }
      ...
      //用户是超级版主
      //用户是系统管理员
      $sql = 'update bbs_subject set subject_status=0 where id=?';
      $sth = $this->db->prepare($sql);
